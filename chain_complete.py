"""JGDChainCompleteAgent — 停止条件二选一: 完整链 或 穷尽证明 (R9)。

用户裁决的停止条件: 挖到完整链, 或在定义的搜索范围内彻底证明没有链。
两者之前 agent 不停; 状态持久化可断点续跑。

图数据源规则 (用户裁决 R10): 图必须根据项目实时构造;
...
public class CB{n} {{
            ...
            for (int ai = 2; ai < args.length; ai++) {{
                ...
                try {{
                    ...
                    for (Field f : bc.getDeclaredFields()) {{
                        ...
                        Class<?> ft = f.getType();
                        Object v;
                        if (fwd != null && f.getName().equals(fwd)
    ...
    // R31 字段契约合成: 除转发字段外全字段注"存活值" — 具体类裸实例/空数组
    ...
    static Object survival(Class<?> ft, Object recv) {{
        try {{
            ...
            // R47(sink参数盲区): 类型化 canary — URL/URI 字段带可观测协议串,
            // 异常消息回显即值流证据
...
def _sink_inventory() -> list[tuple[str, str]]:
    ...
    R12: 仅 14 条 groovy 定义时反向可达近空(pairs=0 的假穷尽) — 扩展种子面。
    """
...
def add_dispatch_edges(gdb: Path, fp: str) -> int:
    ...
    # R47(反射可见性盲区): 反射 API 节点(Class.forName/Method.invoke/lookup)
...
def retag_sinks(gdb: Path, sink_keys: set[tuple[str, str]]) -> int:
    """R12: sink 种子扩展后重打角色(边不动); 返回命中 sink 方法数。"""
    conn = sqlite3.connect(str(gdb))
...
def discover_product_sinks() -> list[tuple[str, str]]:
    """R40: 产品自有 sink 发现 — 目标语料内直接调用危险原语名的方法
...
def ensure_graph(verbose: bool = True) -> Path:
    """R10: 图实时构造规则。指纹匹配的已跑图才复用, 否则从当前语料重建。"""
...
class JGDChainCompleteAgent:
    def __init__(self):
        # R49(语料分域): 状态文件按语料指纹隔离 — 换语料自动归档旧状态,
        ...
        if STATE.exists():
            ...
            if old.get("corpus_fp") not in (None, fp):
                ...
                print(f"[r49] 语料变更: 旧状态归档 -> {old['corpus_fp'][:8]}")
    ...
    def load_bridges(self) -> list[dict]:
        """R12: 桥池 = 链审计 CONFIRM ∪ evolve TRIGGERED(链PoC已证) ∪ verify 存活者。
        ...
        # R20(ds反思: 入口命中率): 57 个 INTERESTING 里每轮只审 12-15,
        ...
        if vs.exists():
            ...
            # R47(深度盲区): 链延伸递归 — DEPTH2 接收者为桥(深度3),
        ...
        for b in sorted(cand.values(), key=lambda x: x["prio"]):
            ...
            if not evd or not evd.get("bridge_detail"):
                continue
            # R13: 接收者桥的 dispatch=目标方法名; 参数桥的 dispatch=触发器名
    ...
    def scan_receivers(self, dispatch: list[str], cap: int = 60) -> list[dict]:
        ...
        R25: 池按 dispatch 集合缓存 —— 381 桥的 dispatch 大量重复
        ...
        for nid_, dist in reach.items():
            ...
            if mname not in dispatch or "$" in (owner or "") or not owner:
                continue
    ...
    def _carrier_java(kind: str, n: int) -> str:
        """R23(ds: 入口命中率): BAVE 只触发 toString —— hashCode/equals 桥
        在 BAVE 下永远不触发。HashMap 载体触发 hashCode(readObject 重哈希);
        put 时的早期触发由 R18 写侧清除吸收, 只剩读侧。"""
        if kind == "BAVE":
            return (
    ...
    def _receiver_hooks_nl(self, receiver_cls: str) -> dict:
        """R50(F5): 用一次构建的名单缓存查 hooks — 消除 O(jars×receivers)。"""
    ...
    def _receiver_hooks(self, receiver_cls: str) -> dict:
        """R19(ds反思#1): 接收者的自定义序列化钩子 —— readObject 自赋值
    ...
    def run_full_chain(self, bridge: dict, receiver: dict, n: int) -> dict:
        ...
        R17 观测升级: 不再只依赖标记物 4 方法 —— 反序列化异常的栈帧里
        ...
        # R28: 单对复跑与批次同规则选 JDK/载体 — ObjLongPair 事故: 批次在
    ...
    def _classify_run(self, out: str, receiver_cls: str, n: int) -> dict:
        ...
        if sink_fire:
            ...
            # R18 修正: 标记物在对象图上唯一可达路径是 BAVE→桥→接收者→字段,
            # 触发即证明两跳全通 —— 无异常≠无链(链走通时恰好没有栈帧)。
            # hop1 栈帧是佐证不是必要条件 (R17 把证据当门槛是回归)。
            v = "CHAIN_DEPTH2"
            tax = "hop2_reach" + ("+hop1_stack" if hop1 else "")
        elif "JGDCANARY" in out:
            # R21: 攻击者控制的 String 流入了接收者的使用点(异常消息回显)
            # —— 参数流真实发生, sink 只差语义匹配
        ...
        # R19: 标记 hop2 是否伴随接收者自定义钩子(假阳性风险标注)
    ...
    def audit_chain(self, bridge, receiver, result) -> dict:
        prompt = (
            ...
            "观测语义说明(R50): dispatch_stack 中的 MV*/jdk.proxy* 帧是观测标记物\n"
    ...
    def _compile_batch_harness(self) -> bool:
        """R24: harness 只编译一次, 全部 (桥×接收者×载体) 复用 (算法提速核心)。"""
        n = self.BATCH_N
    ...
    def _run_batch(self, bridge_cls: str, receivers: list[str],
        ...
        R26: 最小 classpath —— 通配符 classpath 让 JVM 启动时展开 828 jar
        (~30s/次), 只挂本批实际用到的 jar。
        R27: 按桥选 JDK —— 全语料桥含 Java17 类(Artemis 等), 锁 JDK11 会
        ...
        # R47(观测盲区): -Xlog:class+load — 标记物不响也能证明类被加载/初始化
        ...
        # R30: 最小CP可能丢桥的传递依赖(父类/接口在别的jar) — NCDFE 回退全量CP重试
    ...
    def run(self, max_pairs: int = 200) -> str:
        ...
        print("=" * 64)
        # R14: 新运行覆盖旧判定 — 残留 verdict 会让 pair 循环首桥后误 break
        # (R3/R4 的零尝试事故根因: 上一轮 NO_CHAIN 滞留在 state 里)
        self.state["verdict"] = None
        # R14 污染量化处置: 历史 tried_pairs 在缺陷时期产生(计数/覆盖失真),
        # 全量归档并清零重跑 —— 覆盖数字只在修复后的 harness 下有效
        if self.state.get("tried_pairs") and not self.state.get("r14_archive_done"):
            self.state["tried_pairs_r14_archive"] = list(
                self.state.get("tried_pairs", []))
            self.state["tried_pairs"] = []
            self.state["r14_archive_done"] = True
            print(f"[r14] 归档污染期 tried_pairs "
                  f"{len(self.state['tried_pairs_r14_archive'])} 条, 清零重跑")
        ...
        # R16: 空接收者池的显式穷尽陈述 (ds 阻断项: 桥池=0 不能静默)
        for b in bridges:
            if not pools[b["cls"]]:
        ...
        # R35(ds 记账语义): 每桥显式 pool_size/pairs_tried/pairs_total,
        ...
        per_bridge = dict(prev_pb)   # R36: 跨轮合并 — 配对累计, 账本不能每次重建
        for b in bridges:
        ...
        attempted = 0
        # R24: 批处理+并行 (一个JVM/桥/载体跑完全部接收者, 4 worker 并行)
        ...
        with ThreadPoolExecutor(max_workers=4) as ex:
            ...
            for fut in as_completed(futs):
                ...
                for r in batch:
                    ...
                    if v == "CHAIN_SINK":
                        sink_found = True
                # R33: 每批落盘 — 251-fire 事故: 取证阶段超时使整轮成果丢失
                # R51(D1): totals 同批增量更新 — 窗口截断不丢账本显示
        ...
        # R37(ds 收口): 批次后仍有 pairs_tried<pairs_total 的桥 — 单接收者诊断,
        ...
        # 触发对: R33 直接用批次证据(dispatch_stack 已回传)审计, 免单对复跑
        # R50(F9): LLM 审计并行(8线程) + evidence-hash 缓存;
        # R50(F2): 逐条异常隔离, 失败落 audit_failed.jsonl 不炸整轮(rc=0 可续跑)
        from concurrent.futures import ThreadPoolExecutor, as_completed
            ...
            try:
                audit = self.audit_chain(
                    b, {"cls": recv_like["cls"],
                        ...
                        "dist": r.get("dist", 0)},
                    {"verdict": "CHAIN_DEPTH2" if r.get("hop2") else "HOP1_ONLY",
        ...
        with ThreadPoolExecutor(max_workers=8) as ex:
            ...
            for fut in as_completed(futs):
                ...
                self.state["evidence"].append({
                    ...
                    "result": {"verdict": "CHAIN_DEPTH2" if r["hop2"] else "HOP1_ONLY",
    ...
    def reclassify(self) -> int:
        """R18: 用修正后的判定语义重判存量 pair_results。
    ...
    def reflect_with_ds(self) -> dict:
        """R18: 与 ds 联合反思 — 全部失败分类 + hop2 发现 + 盲区质询。"""
        results = self.state.get("pair_results", [])
        ...
        prompt = (
            "你是 JGD 的联合反思审计员。挖掘 agent 的历史: R17 曾把"
    ...
    def audit_fires(self, cap: int = 400) -> int:
        """R34: 补审计 — 批次触发但未及审计的 pair_results(带 dispatch_stack)。"""
    ...
    def entry_side_audit(self) -> dict:
        """R32: 入口侧新颖性专审 — HashMap+hashCode桥 作为通用入口从未被 ds
        正面评估过(此前只评接收者侧)。"""
...
public class MV{n} implements Serializable, InvocationHandler {{
    // R29(ds: 可观测分派栈): 标记物记录自身调用者栈 —— 直接证据
...
public class CC{n} {{
    public static void main(String[] args) {{
        try {{
            ...
            // R21(ds: 值可达性): sink 路径需要参数值(JNDI URL/表达式/路径) —
            ...
            // R18: 清掉写侧(writeObject 钩子)可能触发的标记 —— 只认读侧
