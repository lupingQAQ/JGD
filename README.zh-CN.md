...
- **动态**：批处理并行 JVM 探针（五载体 × 字段契约合成 × 多 JDK）
...
| 主张 | 状态 |
|------|------|
| 公开链：必能检测 | 是 — 机械判定（有限签名集） |
| 未公开链：声明域内穷尽 | 是 — 覆盖账本可复算 |
| 未公开链：全域穷尽 | 否 — 不可判定（Rice 定理） |
| PoC：永远完整产出 | 是 — 尾巴候选循环至 RCE 闭环 |
| 载体家族：全部经典触发路径 | 是 — toString/hashCode/equals/compare/compareTo |
